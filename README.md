...
- **Joker Card (J)** 🃏 — *новое* abstain voting для участников без контекста
...
POST /api/v1/estimates/
{
  ...
  "story_points": 8,
  "user_id": 1,
  "is_joker": false  # NEW: Joker card support
}
...
3. **Joker голоса исключены** — только реальные оценки (1-16) учитываются
...
scores = [4, 4, 8, J]        # Согласовано (valid: [4,4,8]) → ~5 points (J исключен)
scores = [2, 4, 4, 4, 8, J]  # Не согласовано (valid: [2,4,4,4,8], diff=6>2) → обсуждение
scores = [J, J, J]           # Не согласовано (нет реальных оценок) → нельзя оценить
```

## 🃏 Joker Card Feature (NEW)

Читайте подробную документацию:
- **Быстрый старт**: [JOKER_QUICK_START.md](JOKER_QUICK_START.md)
- **Полная документация**: [JOKER_CARD_FEATURE.md](JOKER_CARD_FEATURE.md)
- **Развертывание**: [DEPLOYMENT_CHECKLIST.md](DEPLOYMENT_CHECKLIST.md)
...
- [x] Joker Card (J) support — abstain voting
